...
2. SwiftUI.List: View, Hashable
   ...
   * 첫번째 문제는 List를 통해 만든 View가, ViewModel이 publish를 했음에도 갱신이 되지 않는 이슈가 있었다
      ...
      7. 첫번째 문제는 사실 List에 넘겨주는 모델의 아이디가 identifiable을 채택한 UUID였는데(id: \\.self.id(UUID)), 이게 변화하지 않기 때문에 새로 그려지지 않던 것이었다
      8. 그래서 모델이 Identifiable이 아니라 Hashable을 채택하게 한 뒤 \\.self를 넘겨주었고, 자식뷰에서는 viewModel을 삭제하였는데 아주 깔끔하게 문제가 해결이 되었었다!!!
      9. 라고 생각했으나 두번째 문제가 발생했다...
...
3. Relationship between types within MVVM
...
4. MVVM Architecture
   ...
   * 그럼에도 불구하고, event-driven 방식이 아닌 data-driven 방식을 접해보면서 어떤 자료를 찾아야 하고 어떻게 공부를 해나가야하는지 경험을 해볼 수 있었던 것 같아서 아주 뜻깊은 경험이 아니었나 싶다
...
| remained | hmm...🤔 |
|---|---|
| CloudKit | - 현재 리드미를 작성하는 2021.11.16.을 기준으로 CloudKit을 공부를 하기는 했지만 프로젝트에는 반영을 하진 못한 상태다 <br> - 평소에도 관심이 있었기 때문에 약간 호랑이굴에 정신차리고 들어가는 느낌으로 애플개발자도 큰돈을 들여 결제를 했는데... 너무 많이 아쉬움이 남는다 |
| Combine | - concurrency한 내용들과 Result 타입에 관한 내용을 한번 추상화한 라이브러리라고 이해하고 있는 상태다 <br> - 아주 지엽적인 부분에 대해서는 공부를 하고 사용을 했으나, 공부할 게 너무 많다보니 이 부분에 대해서는 제대로 공부를 하진 못해서 많이 아쉬움이 남는다 <br> - 돌이켜보면 SwiftUI+Combine이 특히 메시지를 전달할 때 Combine을 쓰는 것 같은데 오히려 공부를 못해서 프로젝트가 많이 발전을 못하지 않았나... 아쉬움이 많이 남는다... 제대로 공부를 해봐야겠다 |
| MVVM | - 처음에는 약간 많이들 쓰는 아키텍처인 것 같아서 욕심을 많이 부려서 해보려고 했는데, 힘이 들긴 했지만 재미도 있었다 <br> - 이렇게 리드미를 작성해보면서 생각을 해보니 아키텍처도 종류가 많은데 이게 뭐라고 굳이 이걸 했어야 했나 싶은 생각도 든다 <br> - 다른 아키텍처도 많이 경험해보고 싶다. 어떤 문제를 극복하기 위해 나온 아키텍처인지, 또 그렇게 함으로써 갖게 된 문제점은 어떤 게 있는지... 아키텍처에서 뭔가 인간미를 많이 느낄 수 있는 것 같아서 재밌다. 나같이 고지식한 사람들은 MVC를 좋아할 거 같다 ㅋㅋ MVVM을 좋아하는 사람은 약간 여행에 미쳐있는 사람이 아닐까...|
